...
exports.quickChat = exports.sendMessage = exports.deleteSession = exports.getSession = exports.getSessions = exports.createSession = exports.healthCheck = void 0;
...
const generateWithOllama = async (systemPrompt, turns) => {
    if (!hasOllama()) {
    ...
    try {
        ...
        const wrapped = new Error(`Cannot connect to Ollama at ${OLLAMA_BASE_URL}. Start Ollama and pull model "${OLLAMA_MODEL}".`);
        wrapped.status = 503;
    ...
    if (!response.ok || data?.error) {
        const wrapped = new Error(data?.error || `Ollama request failed with status ${response.status}. Ensure model "${OLLAMA_MODEL}" is available.`);
        wrapped.status = response.status || 500;
        throw wrapped;
    }
    const text = String(data?.response || '').trim();
    if (!text) {
        const wrapped = new Error('Ollama returned empty response');
        wrapped.status = 502;
        throw wrapped;
    }
    return text;
...
const generateAssistantReply = async (systemPrompt, turns) => {
...
const getSystemPrompt = async (userId) => {
    ...
    const userGoal = goal?.goalType === 'WEIGHT_LOSS'
        ? 'Giam can'
