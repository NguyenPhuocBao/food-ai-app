...
const DEFAULT_OUT_DIR = path_1.default.resolve(process.cwd(), 'exports', 'fine-tune-all');
const SYSTEM_PROMPT = 'Ban la AI health coach chuyen ve dinh duong va suc khoe. Tra loi ngan gon, an toan, khong chan doan y khoa.';
...
const valRatio = Number.isFinite(requestedValRatio) && requestedValRatio > 0 && requestedValRatio < 0.3
    ? requestedValRatio
    : DEFAULT_VAL_RATIO;
const requestedTestRatio = Number(arg('test-ratio') || DEFAULT_TEST_RATIO);
const testRatio = Number.isFinite(requestedTestRatio) && requestedTestRatio > 0 && requestedTestRatio < 0.2
    ? requestedTestRatio
    : DEFAULT_TEST_RATIO;
const outDir = arg('out-dir') ? path_1.default.resolve(process.cwd(), String(arg('out-dir'))) : DEFAULT_OUT_DIR;
...
const buildQuestionVariant = (question, seed) => {
    const mode = Math.abs(seed) % 10;
    const base = mutateText(question, seed);
    if (mode === 0)
        return base.endsWith('?') ? base : `${base}?`;
    if (mode === 1)
        return `${pick(questionStarters, seed)} ${base.toLowerCase()} ${pick(questionClosers, seed + 7)}`;
    if (mode === 2)
        return `${base} Neu toi muon ap dung ngay hom nay thi bat dau tu dau?`;
    if (mode === 3)
        return `Ban phan tich giup: ${base.toLowerCase()} De toi de theo doi trong app?`;
    if (mode === 4)
        return `${base} Ban co the de xuat phien ban don gian hon khong?`;
    if (mode === 5) {
...
const buildAnswerVariant = (answer, seed) => {
    const mode = Math.abs(seed) % 7;
    const core = mutateText(answer, seed + 17);
    if (mode === 0)
        return core;
    if (mode === 1)
        return `${pick(answerIntros, seed)} ${core}`;
    if (mode === 2)
        return `${core} ${pick(answerOutros, seed + 3)}`;
    if (mode === 3)
        return `${pick(answerIntros, seed + 5)} ${core} ${pick(answerOutros, seed + 11)}`;
    if (mode === 4)
        return `${pick(answerIntros, seed + 9)} ${core} ${pick(answerStructs, seed + 12)}`;
    if (mode === 5)
        return `${core} Dieu kien ap dung: ${pick(constraints, seed + 13)}. ${pick(answerOutros, seed + 14)}`;
    return `${pick(answerStructs, seed + 21)} ${core} ${pick(answerOutros, seed + 22)}`;
};
const augment = (baseExamples, target) => {
    const out = [];
    const dedupe = new Set();
...
const run = async () => {
    ...
    if (baseExamples.length === 0) {
        baseExamples = normalize(chatbot_training_defaults_1.DEFAULT_CHATBOT_TRAINING_EXAMPLES);
    ...
    const trainPath = path_1.default.join(outDir, 'chatbot-all.train.jsonl');
    const valPath = path_1.default.join(outDir, 'chatbot-all.val.jsonl');
    const testPath = path_1.default.join(outDir, 'chatbot-all.test.jsonl');
    const reportPath = path_1.default.join(outDir, 'chatbot-all.report.json');
    const rawPath = path_1.default.join(outDir, 'chatbot-all.raw.json');
    writeJsonl(trainPath, trainRows);
    writeJsonl(valPath, valRows);
    writeJsonl(testPath, testRows);
    fs_1.default.writeFileSync(rawPath, JSON.stringify(expandedExamples, null, 2), 'utf8');
    if (shouldSyncDb) {
