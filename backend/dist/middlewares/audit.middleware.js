...
exports.auditMutationMiddleware = void 0;
...
const inferEntity = (pathWithoutQuery) => {
    ...
    if (module === 'admin') {
        ...
        if (resource === 'users' && segments.includes('meal-plans'))
            return 'MealPlan';
        if (resource === 'meals')
            return 'Meal';
...
const toSafeValue = (value, depth = 0) => {
    ...
    if (Array.isArray(value)) {
        ...
        if (value.length > 30)
            capped.push(`[+${value.length - 30} items]`);
        return capped;
    }
...
const shouldSkipAutoAudit = (method, pathWithoutQuery) => {
    if (!MUTATING_METHODS.has(method))
        return true;
    if (!pathWithoutQuery.startsWith('/api/'))
        return true;
    if (pathWithoutQuery.startsWith('/api/admin/audit-logs'))
        return true;
    ...
    res.on('finish', () => {
        try {
            if (res.statusCode >= 400)
                return;
            if (shouldSkipAutoAudit(method, pathWithoutQuery))
                return;
            if (!req.user?.id)
                return;
            ...
            const entityId = inferEntityId((req.params || {}));
            ...
            void prisma_1.default.auditLog.create({
                data: {
                    userId: req.user.id,
                    action,
                    entity,
