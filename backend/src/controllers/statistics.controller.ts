...
type OverviewGroupBy = 'day' | 'week' | 'month';

const parseGroupBy = (raw: unknown): OverviewGroupBy => {
...
const getWeekStartFromDayStart = (dayStart: Date) => {
  const localDate = new Date(dayStart.getTime() + VN_UTC_OFFSET_HOURS * 60 * 60 * 1000);
  const weekday = localDate.getUTCDay();
  return shiftAppDays(dayStart, -weekday);
};

export const getNutritionOverview = async (req: any, res: Response) => {
  try {
    ...
    const nutritionMap = new Map(
      nutritionRows.map((row) => [toVnDateKey(row.date), row]),
    );

    const daily = Array.from({ length: windowDays }, (_, index) => {
      const dayStart = shiftAppDays(startDate, index);
    ...
    daily.forEach((item) => {
      ...
      if (groupBy === 'week') {
        ...
        bucketEnd = shiftAppDays(bucketStart, 6);
      ...
      existing.days += 1;
      if (item.hasLog) existing.loggedDays += 1;
    ...
    const bestBucket = bestBucketSource.reduce((best, bucket) => (
      bucket.calories > best.calories ? bucket : best
    ), bestBucketSource[0] || null);
    const worstBucket = bestBucketSource.reduce((worst, bucket) => (
      bucket.calories < worst.calories ? bucket : worst
    ), bestBucketSource[0] || null);

    let streak = 0;
    for (let index = daily.length - 1; index >= 0; index -= 1) {
      if (!daily[index].hasLog) break;
      streak += 1;
    }
