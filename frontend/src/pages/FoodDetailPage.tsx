      ...
      if (isFavorite) {
        ...
        setFood((current) =>
          current
            ? {
                ...current,
                favorites: current.favorites?.filter((favorite) => favorite.userId !== user.id) || [],
              }
            : current
        );
        ...
        setFood((current) =>
          current
            ? {
                ...current,
                favorites: [...(current.favorites || []), { id: Date.now(), userId: user.id, foodId: current.id }],
              }
            : current
        );
            ...
            <div className="flex items-center gap-2 mt-4">
              {Array.from({ length: 5 }).map((_, index) => (
                <Star
                  key={index}
                  size={18}
                  className={index < Math.round(ratingAverage) ? 'fill-amber-400 text-amber-400' : 'text-gray-200'}
                />
              ))}
              <span className="font-black text-gray-900">{ratingAverage.toFixed(1)}</span>
            </div>
            <p className="text-sm text-gray-500 mt-3">
              {food.favorites?.length || 0} người dùng đã lưu món này trong thư viện yêu thích.
            </p>
          </div>
      ...
      {food.recipe ? (
        ...
        <section className="bg-white rounded-[32px] border border-gray-100 shadow-sm p-10 text-center">
          ...
          <p className="text-gray-500 mt-3">
            Bạn vẫn có thể thêm món vào nhật ký, hoặc quay lại thư viện để xem các món khác đã có hướng dẫn nấu.
          </p>
            ...
            {food.reviews.slice(0, 6).map((review) => (
              <div key={review.id} className="rounded-[24px] border border-gray-100 p-5">
                ...
                <p className="text-sm text-gray-600 mt-4 leading-relaxed">
                  {review.comment || 'Người dùng chưa để lại nhận xét chi tiết.'}
                </p>
              </div>
            ))}
          </div>
